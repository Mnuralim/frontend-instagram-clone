  ...
  return (
    <section className="bg-black w-[360px] rounded-md hidden md:block">
      <div className="flex items-center gap-3 pt-3 px-3">
        <Link aria-label="link" href={`/${user._id}/?tab=post`} scroll={false} className="rounded-full min-w-[57px]">
          <Image
        ...
        <div>
          <Link
            ...
            scroll={false}
            className="font-semibold hover:text-[#A8A8A8]"
          >
            {user.username}
          </Link>
      ...
      <div className="grid grid-cols-3 gap-[2px] mt-3">
        {posts?.map((post) =>
          post.type === 'post' ? (
            <Link aria-label="link" key={post._id} href={`/p/${post._id}`} className="aspect-square hover:opacity-50">
              <Image
                src={post.media}
                alt={post.caption}
                width={5000}
            ...
            post.type === 'reel' && (
              <Link aria-label="link" href={`/p/${post._id}`} key={post._id} className="aspect-square hover:opacity-50">
