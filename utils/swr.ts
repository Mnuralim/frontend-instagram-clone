  ...
  const { data, isLoading, mutate } = useSWR(
    token
      ? id
        ...
        : [`${process.env.API_URL}/users/?suggestion=${suggestionQuery}&username=${usernameQuery}`, token]
      : null,
...
export const usePost = (token: string, id?: string, query?: IPostQuery) => {
  ...
  const { data, isLoading, mutate, isValidating } = useSWR(
    token
      ? id
        ? [`${process.env.API_URL}/posts/${id}`, token]
        : [`${process.env.API_URL}/posts/?author=${authorQuery}&limit=${limitQuery}`, token]
      : null,
    ([url, token]) => fetcher(url, token)
  )
  return {
...
export const useHistory = (token: string) => {
  const { data, isLoading, mutate } = useSWR(
    token ? [`${process.env.API_URL}/histories/`, token] : null,
    ([url, token]) => fetcher(url, token)
  )
  return {
...
export const useComment = (token: string, postId: string) => {
  const { data, isLoading, mutate } = useSWR(
    token ? [`${process.env.API_URL}/comments/${postId}`, token] : null,
    ([url, token]) => fetcher(url, token)
  )
  return {
...
export const useLikes = (token: string, id: string) => {
  const { data, isLoading, mutate } = useSWR(
    token ? [`${process.env.API_URL}/posts/likes/${id}`, token] : null,
    ([url, token]) => fetcher(url, token)
  )
  return {
    likes: data?.allLikes,
