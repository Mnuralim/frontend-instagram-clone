    ...
    <div>
      <div className="flex items-center justify-between gap-3 w-full mt-7 px-3 md:px-8">
        <div className="md:w-1/3">
          <Image src={user?.profile.imageProfile} alt={user?.username || "profile"} width={80} height={80} className="object-center w-20 h-20 rounded-full md:w-36 md:h-36" />
        </div>
        <div className="md:w-2/3 self-center">
          {isMyProfile ? (
            <div className="items-center w-full gap-3 mb-5 hidden md:flex">
              <p className="text-lg">{user?.username}</p>
              <Link href={`/profile/edit/${user?.username}`} className="bg-[#262626] py-1 w-28 text-center rounded-lg font-semibold hover:bg-[#151515]">
                Edit profile
              </Link>
              <button className="bg-[#262626] py-1 w-28  text-center rounded-lg font-semibold hover:bg-[#151515]">Share profile</button>
            </div>
          ) : (
            <div className="items-center w-full gap-3 mb-5 hidden md:flex">
              <p className="text-lg">{user?.username}</p>
              <button onClick={handleFollowUser} className={`${user?.alreadyFollow ? "bg-[#262626]" : "bg-[#0195f7]"} py-1 w-28 text-center rounded-lg font-semibold hover:bg-[#151515]`}>
                {user.alreadyFollow ? "Following" : "Follow"}
              </button>
              <button className="bg-[#262626] py-1 w-28 text-center rounded-lg font-semibold hover:bg-[#151515]">Message</button>
      ...
      {isMyProfile ? (
        <div className="flex items-center gap-1 my-3 px-3 md:hidden">
          <Link href={`/profile/edit/${user?.username}`} className="bg-[#262626] py-1 w-[45%] text-center rounded-lg font-semibold hover:bg-[#151515]">
            Edit profile
          </Link>
          <button className="bg-[#262626] py-1 w-[45%] text-center rounded-lg font-semibold hover:bg-[#151515]">Share profile</button>
          <button className="bg-[#262626] py-1 w-[9%]  rounded-lg font-semibold hover:bg-[#151515]">
            <HiOutlineUserAdd className="text-center inline-block transform scale-x-[-1]" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-1 my-3 px-3 md:hidden">
          <button onClick={handleFollowUser} className={`${user?.alreadyFollow ? "bg-[#262626]" : "bg-[#0195f7]"} py-1 w-[45%] text-center rounded-lg font-semibold`}>
            {user.alreadyFollow ? "Following" : "Follow"}
          </button>
          <button className="bg-[#262626] py-1 w-[45%] text-center rounded-lg font-semibold hover:bg-[#151515]">Message</button>
          <button className="bg-[#262626] py-1 w-[9%]  rounded-lg font-semibold hover:bg-[#151515]">
            <HiOutlineUserAdd className="text-center inline-block transform scale-x-[-1]" />
          </button>
        </div>
