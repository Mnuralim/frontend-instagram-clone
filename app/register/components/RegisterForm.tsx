...
const RegisterForm = ({ inputValue, isLoading, handleChangeValue, handleSubmit }: Props) => {
  ...
  const isDisabled = inputValue.email.length === 0 || inputValue.password.length === 0 || inputValue.username.length === 0 || inputValue.password.length < 6;

  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-1 mt-5">
      <div className="mb-4">
        <input type="text" name="username" id="username" placeholder="Username" className="w-full bg-[#262626] py-3 px-3 outline-none rounded-md" value={username} onChange={(e) => setUsername(e.target.value)} required />
      </div>
      <div>
        <input
          type="email"
